...
- **Phase 1 (MVP) — scaffolded**: accounts, profiles, resumes, jobs, ingestion (Adzuna + Greenhouse), matching, notifications, applications, tailoring, agent skeleton, interview, credentials, extension_api skeleton, vault skeleton, billing skeleton, streaming. Google OAuth wired (provider + view + tests + frontend callback).
- **Phase 2 — pending**: Jooble/JSearch/Lever adapters, Playwright scraper, email-forward parser, web-search tool, browser extension, cover-letter generation, JD red-flag detector, response-rate analytics.
- **Phase 3 — pending**: autonomous-apply with portal AuthFlows, LinkedIn integration, salary intelligence, voice mode for the interview agent.
