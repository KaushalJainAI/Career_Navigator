...
- **Phase 1 (MVP) — implemented beyond scaffold**: accounts (incl. Google OAuth + API tokens), profiles, resumes, jobs, ingestion (Adzuna + Greenhouse), matching, notifications, applications, networking, tailoring (incl. cover letters), agent, interview (Grill text mode + post-session report), credentials, extension_api, vault skeleton, billing (credit ledger; Stripe checkout still disabled), streaming, ai (LLM provider transports), portals (browser-driven no-API scraping). 19 apps total.
- **Phase 2 — partially implemented**: Jooble/JSearch/Lever adapters landed (code + tests; live-key smoke pending); tiered apply (assist/autofill/autonomous) with HITL approval tokens; MV3 extension autofill bridge + CI; **Ghost-Job Shield** (flagship — `jobs/ghost.py` deterministic risk scorer wired into the ingestion upsert, exposed on the jobs API + UI, deprioritized in apply-prepare; subsumes the JD red-flag detector); **match explainability** (`matching/scorer.py` returns matched/missing skills + colour-coded reasons, persisted on `MatchScore` and rendered on job detail); **ATS-safe résumé export** (`resumes/ats_export.py` deterministic single-column plain-text + .docx builders, `GET /api/v1/tailoring/resume/export/?fmt=txt|docx`, with download buttons on job detail); **response-rate analytics** (`applications/analytics.py` funnel + response/offer rates + per-tier breakdown + avg-days-to-response over event history, `GET /api/v1/applications/analytics/`, dashboard panel); the **`portals` app** — browser-driven, in-session scraping of the no-API sources (LinkedIn/Naukri/Unstop/YC) via an injectable `BrowserDriver` (real `PlaywrightDriver` + `FakeBrowserDriver` for tests), per-user encrypted sessions with env-cookie fallback, scrapers that emit the canonical posting dict into `upsert_postings`; expanded Playwright E2E across the core flows. **Still pending**: live per-portal selector tuning (the `# LIVE-SELECTOR` markers), JobSpy-wrapped adapter, email-forward parser, web-search tool, extension one-click job capture, truthfulness verification pass. See [docs/competitive-landscape.md](docs/competitive-landscape.md) for the prioritisation rationale.
