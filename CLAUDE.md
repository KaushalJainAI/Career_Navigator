...
- **Phase 2 — partially implemented**: Jooble/JSearch/Lever adapters landed (code + tests; live-key smoke pending); tiered apply (assist/autofill/autonomous) with HITL approval tokens; MV3 extension autofill bridge + CI. **Still pending**: JobSpy-wrapped adapter, Playwright scraper, email-forward parser, web-search tool, extension one-click job capture, Ghost-Job Shield + JD red-flag detector, match-explainability UI, truthfulness verification pass, ATS-safe resume export, response-rate analytics. See [docs/competitive-landscape.md](docs/competitive-landscape.md) for the prioritisation rationale.
