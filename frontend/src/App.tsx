...
import { Briefcase, LayoutDashboard, Send, FileText, Mic2, LogOut, Compass, Bell, Settings } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="ml-auto flex min-w-0 items-center gap-2">
            ...
            <Link
              to="/profile"
              className="hidden min-w-0 items-center gap-3 rounded-2xl bg-slate-100 px-3 py-2 transition hover:bg-slate-200 sm:flex"
              title="Your profile"
            >
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs font-semibold text-slate-500 capitalize">{user.cn_profile?.tier ?? 'Free'} Tier</p>
              </div>
            </Link>
            <Link
              to="/settings"
              className={`grid h-11 w-11 place-items-center rounded-2xl ${
                location.pathname.startsWith('/settings') ? 'bg-slate-950 text-teal-300' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-950'
              }`}
              aria-label="Settings"
              title="Settings"
            >
